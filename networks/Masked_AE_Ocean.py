...
class Masked_Ocean(nn.Module):
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            img_size, patch_size, self.N_in_channels, self.embed_dim
        )
        ...
        self.dpr_decoder = [
            x.item() for x in torch.linspace(0, drop_path_rate, params.decoder_depth)
        ]
    ...
    def initialize_weights(self):
        ...
        decoder_pos_embed = get_2d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], (self.h, self.w), cls_token=False
        )
    ...
    def patchify(self, imgs, channel):
        ...
        x = x.reshape(
            shape=(
                imgs.shape[0],
                self.h * self.w,
                self.patch_size[0] * self.patch_size[1] * channel,
            )
        )
        return x
