...
def weighted_acc_masked(pred, target, weighted=True, maskarray=1):
    ...
    pred -= mean(pred)
    target -= mean(target)
    s = np.sum(np.cos(np.pi / 180 * lat_torch(np.arange(0, num_lat), num_lat)))
    weight = (
...
def latitude_weighting_factor_torch(
    ...
    return num_lat * torch.cos(3.1416 / 180.0 * lat_torch(j, num_lat)) / s


def latitude_weighting_factor(j, num_lat, s):
    return num_lat * np.cos(3.1416 / 180.0 * lat_np(j, num_lat)) / s
...
def weighted_rmse_torch_channels(
    ...
    s = torch.sum(torch.cos(3.1416 / 180.0 * lat_torch(lat_t, num_lat)))
...
def weighted_rmse(pred, target):
    ...
    s = np.sum(np.cos(3.1416 / 180.0 * lat_np(lat_t, num_lat)))
    weight = np.reshape(latitude_weighting_factor(lat_t, num_lat, s), (1, 1, -1, 1))
    result = np.sqrt(np.nanmean(weight * (pred - target) ** 2.0, axis=(-1, -2)))
    return result
...
def weighted_acc(pred, target):
    ...
    s = np.sum(np.cos(3.1416 / 180.0 * lat_np(lat_t, num_lat)))
    weight = np.reshape(latitude_weighting_factor(lat_t, num_lat, s), (1, 1, -1, 1))
    result = np.nansum(weight * pred * target, axis=(-1, -2)) / np.sqrt(
        np.nansum(weight * pred * pred, axis=(-1, -2))
        * np.nansum(weight * target * target, axis=(-1, -2))
    )
    return result
