...
from networks.fourier_KNO import AutoencoderViT

os.environ["WANDB_API_KEY"] = "a7e342091ff8ea4735bde79086f58309c7157a18"
...
class Trainer():
    ...
    def __init__(self, params, world_rank):
        ...
        # 加载模型
        self.model = AutoencoderViT(params).to(self.device)
    ...
    def train(self):
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            if self.params.log_to_screen:
                ...
                logging.info('Train loss: {}. Gen loss: {}. Rec loss: {}. Valid loss: {}'.format(train_logs['loss'], train_logs['loss_gen'], train_logs['loss_rec'], valid_logs['valid_loss']))
    ...
    def train_one_epoch(self):
        ...
        for i, data in enumerate(self.train_data_loader, 0):
            ...
            with amp.autocast(self.params.enable_amp):
                gen, rec = self.model(inp)
                gen.to(self.device, dtype=torch.float)
                rec.to(self.device, dtype=torch.float)
                loss_gen = self.loss_obj(gen,tar)
                loss_rec = self.loss_obj(rec,inp[:,0:self.params.out_chans])
                loss = self.params.weight_gen_loss * loss_gen + \
                       self.params.weight_rec_loss * loss_rec
        ...
        logs = {'loss': loss, 'loss_gen': loss_gen, 'loss_rec': loss_rec}
    ...
    def validate_one_epoch(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.valid_data_loader, 0):
                ...
                os.mkdirs(params['experiment_dir'] + "/" + str(i), exist_ok =True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", default='autoencoder_vit', type=str)
    ...
    params['name'] = args.config + '_' + str(args.run_num)  # 实验名称
    params['group'] = "HYCOM_ERA5_1degree" + args.config  #
    params['project'] = "HYCOM_ERA5_1degree"  # 项目名称
    params['entity'] = "ai4science_ocean_AI_model"  # wandb上对应的team名称
