...
def plot_global_sss(
    ...
    var_idx = VAR_INDEX_15_LEVELS['S0']
    ...
    # color
    mycolor=['#E0F2A4','#AFD9A9','#7BBAA9','#4E7DB3','#5D5199','#3C1266']
    ...
    if projection == 'PlateCarree':
        ...
        cf = ax.pcolormesh(longitude2d, latitude2d, data, cmap=cmap, vmin=32.0, vmax=38.0, 
        ...
        c.set_ticks(np.arange(32, 38.00001,0.5)) #自定义数字间隔
        ...
        cf = ax.pcolormesh(longitude2d, latitude2d, data, cmap=cmap, vmin=-2.0, vmax=32.0, 
                           transform=ccrs.PlateCarree(central_longitude=180))
...
if __name__ == '__main__':
    ...
    projection = 'robinson' # 投影方式，可选："robinson"，"PlateCarree"
    pre_or_truth = 'truth'    # 可视化预报还是真值
    init_condition = 1      # 第几次起报
    interval  = 1           # downscale, 经纬度间隔, 默认为1
    time_step = 5           # 第几个预报时间步
    save_path = os.path.join(file_dir, f'global_sss_{projection}_init_{init_condition}_timestep_{time_step}_{pre_or_truth}.png')  # 图像保存路径
    plot_global_sss(file_path           = file_path,
