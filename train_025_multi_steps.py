...
class Trainer():
    ...
    def __init__(self, params, world_rank):
        ...
        self.train_data_loader, self.train_dataset, self.train_sampler = get_data_loader(params, params.train_data_path,
                                                                                         dist.is_initialized(),
                                                                                         train=True)
        self.valid_data_loader, self.valid_dataset, self.valid_sampler = get_data_loader(params, params.valid_data_path,
                                                                                         dist.is_initialized(), 
                                                                                         train=True)
    ...
    def train_one_epoch(self):
        ...
        for i, data in enumerate(self.train_data_loader, 0):
            ...
            if params.fusion_3d_2d:
                ...
                # logging.info(f'inp_3d: {inp_3d.shape}, inp_2d: {inp_2d.shape}, tar:{tar.shape}')
            else:
            ...
            if self.params.orography and self.params.multi_steps_finetune > 1:
                orog = inp[:,-1]
                orog = torch.unsqueeze(orog, dim=1)
            ...
            if self.params.multi_steps_finetune == 2:
                ...
                with amp.autocast(self.params.enable_amp):
                    # step 1
                    gen_step_1  = self.model(inp).to(self.device, dtype = torch.float)
                    tar_step_1  = tar[:, 0, 0:self.params.N_out_channels].to(self.device, dtype=torch.float)
                    ...
                    if self.params.orography:
                        ...
                        # logging.info(f'gen_step_1: {gen_step_1.shape}, atmos_force_orog: {atmos_force_orog.shape}')
                        gen_step_2 = self.model( torch.cat( (gen_step_1, atmos_force, orog), axis = 1)  ).to(self.device, dtype = torch.float)
                    else:
                        atmos_force = tar[:, 0, -5:].to(self.device, dtype=torch.float)
                        gen_step_2 = self.model( torch.cat( (gen_step_1, atmos_force), axis = 1) ).to(self.device, dtype = torch.float)

                    tar_step_2  = tar[:, 1, 0:self.params.N_out_channels].to(self.device, dtype=torch.float)
        ...
        with torch.no_grad():
            for i, data in enumerate(self.valid_data_loader, 0):
                ...
                if params.fusion_3d_2d:
                    ...
                    inp, tar = map(lambda x: x.to(self.device, dtype=torch.float), data)
                    gen = self.model(inp)
                    # logging.info(f'inp: {inp.shape}, tar: {tar.shape}, gen: {gen.shape}')
                gen.to(self.device, dtype=torch.float)

                if self.params.multi_steps_finetune > 1:
                    tar = tar[:, 0, 0:self.params.N_out_channels] 
                # logging.info(f'inp: {inp.shape}, tar: {tar.shape}, gen: {gen.shape}')
