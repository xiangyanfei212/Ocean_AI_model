...
class Koopman_Operator2D(nn.Module):
    def __init__(self, op_size, modes_x, modes_y):
        ...
        self.koopman_matrix_real = nn.Parameter(self.scale * torch.rand(op_size, op_size, self.modes_x, self.modes_y, dtype=torch.float32))
        self.koopman_matrix_imag = nn.Parameter(self.scale * torch.rand(op_size, op_size, self.modes_x, self.modes_y, dtype=torch.float32))

        self.sparsity_threshold = 0.01
    def time_marching(self, input, weights):
        # (batch, t, x,y ), (t, t+1, x,y) -> (batch, t+1, x,y)
        return torch.einsum("bxyt,tfxy->bxyf", input, weights)
    ...
    def forward(self, x):
        ...
        koopman_matrix = torch.view_as_complex(torch.stack([self.koopman_matrix_real, self.koopman_matrix_imag], dim=-1))
        # Fourier Transform
        x = torch.fft.rfft2(x, dim=(1, 2), norm="ortho")
        # Koopman Operator Time Marching
        out_ft = torch.zeros(x.shape, dtype=torch.cfloat, device = x.device)
        out_ft[:, :self.modes_x, :self.modes_y] = self.time_marching(x[:, :self.modes_x, :self.modes_y], koopman_matrix)
        out_ft[:, -self.modes_x:, :self.modes_y,] = self.time_marching(x[:, -self.modes_x:, :self.modes_y], koopman_matrix)
        #Inverse Fourier Transform
        x = torch.fft.irfft2(out_ft, s=(H, W), dim=(1,2), norm="ortho")
...
#         self.mask_token = nn.Parameter(torch.zeros(1, 1, params.decoder_embed_dim))
        ...
        self.koopman_layer = nn.ModuleList([
            Koopman_Operator2D(self.embed_dim, self.modes_h, self.modes_w)
            for i in range(params.koopman_depth)])
...
#         x = torch.tanh(x)
    ...
    def koopman_core(self, x):
        B = x.shape[0]
        x = x.reshape(B, self.h, self.w, self.embed_dim)
        # apply Koopman Layers
        for koopman in self.koopman_layer:
            x1 = koopman(x)
            x = torch.tanh(x + x1)
        x = x.reshape(B, self.h*self.w, self.embed_dim)
