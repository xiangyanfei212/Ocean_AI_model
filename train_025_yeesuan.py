            ...
            with amp.autocast(self.params.enable_amp):
                ...
                if self.params.land_mask:
                    ...
                    with h5py.File(self.params.land_mask_path, 'r') as _f: 
                        ...
                        mask_data = torch.tensor(np.array(_f['fields']), device=self.device, dtype=torch.bool)
...
if __name__ == '__main__':
    ...
    #dist.init_process_group(backend='nccl', init_method=args.dist_url,world_size=args.world_size, rank=args.comm_rank)
    dist.init_process_group(backend='nccl')
    #获取本地节点的GPU数量
    local_rank = dist.get_rank() % 4
    ...
    print("world_size:",world_size)
    print("int(params.batch_size // world_size):",int(params.batch_size // world_size))
    params['batch_size'] = int(params.batch_size // world_size)  # batch size must be divisible by the number of gpu's
